...
class Utils {
    ...
    /**
     * Copy file from directory FROM to directory TO,
     * with NAMEOFFILE being the name of the file.
     * FINALNAME is the name of the file at end.
     */
    static void copyFile(File to, File from,
                         String nameOfFile, String finalName) {
        File current = Utils.join(from, nameOfFile);
        if (!current.isFile()) {
            throw new GitletException("Trying to copy "
                    + "a file that doesnt exist");
        }
        byte[] contents = Utils.readContents(current);
        Utils.writeContents(Utils.join(to, finalName), contents);
    }
